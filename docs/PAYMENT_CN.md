...
Sub2API 内置支付系统，支持用户自助购买余额和订阅套餐，无需部署独立的支付服务。
...
|--------|------|--------|
| **启用支付** | 启用或禁用支付系统 | 关闭 |
| **开放余额购买** | 允许用户购买账户余额 | 开启 |
| **开放订阅购买** | 允许用户购买订阅套餐 | 开启 |
| **商品名前缀** | 支付页面显示的商品名前缀 | - |
| **商品名后缀** | 商品名后缀（如"元"） | - |
| **最低金额** | 单笔最低充值金额 | 1 |
...
1. 在 **设置 → 支付设置** 中使用 **开放订阅购买**，全局开放或关闭订阅购买流程。
2. 在 **订单 → 订阅套餐** 中，对每个套餐使用 **可售卖** 开关，选择用户可以购买哪些套餐。

关闭订阅购买后，结算接口不会返回套餐，直接创建订阅订单也会被拒绝。关闭余额购买只会独立影响余额充值。已有订阅不会受到影响；这些开关只限制新的购买。
...
- 确保服务器防火墙允许支付平台的回调请求
- 系统会自动进行签名验证，防止伪造回调
- 余额订单支付成功后自动到账；订阅订单支付成功后自动分配或续期所选套餐
...
```
用户选择购买余额金额或订阅套餐，再选择支付方式
       │
       ▼
  创建订单 (PENDING)
  ├─ 校验购买类型、开放开关、待支付订单数和每日限额
  ├─ 负载均衡选择服务商实例
  └─ 调用服务商获取支付信息
       │
       ▼
...
| PAID | 已支付，等待余额到账或订阅履约 |
| COMPLETED | 已完成，余额已到账或订阅已生效 |
| `EXPIRED` | 已过期，超时未支付 |
