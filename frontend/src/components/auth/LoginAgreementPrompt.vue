<template>
    ...
    <div class="flex items-start gap-2">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-[13px] leading-5 text-gray-600 dark:text-dark-300">
          ...
          >
            {{ t('auth.agreementPrompt.agreeRead') }}
          </label>
  ...
  >
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="font-medium">{{ t('auth.agreementPrompt.mustAgree') }}</p>
        <p class="mt-1 text-primary-700 dark:text-primary-200/80">
          {{ t('auth.agreementPrompt.disabledHint') }}
        </p>
      </div>
      <button
        type="button"
        ...
        @click="emit('open')"
      >
        {{ t('auth.agreementPrompt.viewTerms') }}
      </button>
    </div>
  </div>
        ...
        <div class="w-full max-w-[600px] overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/10 dark:bg-dark-900 dark:ring-white/10">
              ...
              <div class="min-w-0 flex-1">
                <div class="flex flex-wrap items-center gap-2">
                  <h2 class="text-xl font-bold tracking-normal text-gray-950 dark:text-white">
                    {{ t('auth.agreementPrompt.updateTitle') }}
                  </h2>
                  <span
                    v-if="updatedAt"
                ...
                <p class="mt-2 text-sm leading-6 text-gray-600 dark:text-dark-300">
                  {{ t('auth.agreementPrompt.updateBody', { date: updatedAt || t('auth.agreementPrompt.recently') }) }}
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="max-h-[58vh] overflow-y-auto px-6 py-5">
            <div class="mb-3 flex items-center justify-between gap-3">
              <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('auth.agreementPrompt.relatedDocs') }}</p>
            </div>
          ...
          <div class="border-t border-gray-100 bg-gray-50/80 px-6 py-4 dark:border-dark-800 dark:bg-dark-950/60">
            <div class="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                @click="emit('accept')"
              >
                {{ t('auth.agreementPrompt.agreeContinue') }}
              </button>
            </div>
          </div>
        </div>
