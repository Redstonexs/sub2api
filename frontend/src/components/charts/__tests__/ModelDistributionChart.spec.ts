...
import { CHART_OTHER, seriesColor } from '@/utils/chartTheme'
...
describe('ModelDistributionChart', () => {
  ...
  it('renders Others in the spending ranking table and uses a dedicated chart color', async () => {
    ...
    expect(chartData.datasets[0].backgroundColor[0]).toBe(seriesColor(0))
    expect(chartData.datasets[0].backgroundColor[2]).toBe(CHART_OTHER)
