<template>
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      <router-link
        :to="homePath"
        class="sidebar-logo flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl border border-gray-200 transition-opacity hover:opacity-80 dark:border-dark-700"
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    // nav.myAnnouncements, not nav.announcements: that key is already bound to the
    // admin announcements management entry.
    { path: '/announcements', label: t('nav.myAnnouncements'), icon: BellIcon },
