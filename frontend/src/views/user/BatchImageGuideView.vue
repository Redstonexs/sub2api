...
async function cancelSelected() {
  ...
  const key = keyForSelectedBatch() || requireApiKey()
  if (!key) return
  if (!(await confirm({ title: t('common.cancel'), message: batchImageText('cancelConfirm'), danger: true }))) return
...
async function deleteJob(job: BatchImageJobRow) {
  ...
  if (!key) return
  if (!(await confirm({ title: t('common.delete'), message: batchImageText('deleteConfirm'), danger: true }))) return
...
async function deleteSelectedJobs() {
  ...
  if (!(await confirm({ title: t('common.delete'), message: batchImageText('deleteSelectedConfirm'), danger: true }))) return
