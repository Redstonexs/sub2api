...
const saveRuntimeConfig = async () => {
  ...
  try {
    ...
    appStore.showError(err?.response?.data?.detail || t('admin.ops.systemLog.saveConfigFailed'))
  } finally {
...
const resetRuntimeConfig = async () => {
  const ok = await confirm({ title: t('admin.ops.systemLog.title'), message: t('admin.ops.systemLog.confirmRollback'), danger: true })
  if (!ok) return
...
const cleanupCurrentFilter = async () => {
  const ok = await confirm({ title: t('admin.ops.systemLog.title'), message: t('admin.ops.systemLog.confirmCleanup'), danger: true })
  if (!ok) return
  try {
    const payload = {
    ...
    appStore.showError(
      extractApiErrorMessage(err, t('admin.ops.systemLog.cleanupFailed'), {
  ...
  <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-900/60">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700 dark:bg-dark-700 dark:text-gray-200">{{ t('admin.ops.systemLog.queue') }} {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700 dark:bg-dark-700 dark:text-gray-200">{{ t('admin.ops.systemLog.written') }} {{ health.written_count }}</span>
        <span class="rounded-md bg-amber-100 px-2 py-1 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">{{ t('admin.ops.systemLog.dropped') }} {{ health.dropped_count }}</span>
        <span class="rounded-md bg-red-100 px-2 py-1 text-red-700 dark:bg-red-900/30 dark:text-red-300">{{ t('admin.ops.systemLog.failed') }} {{ health.write_failed_count }}</span>
      </div>
    </div>

    <div class="mb-4 rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/70">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-gray-700 dark:text-gray-200">{{ t('admin.ops.systemLog.runtimeConfigTitle') }}</div>
          ...
          <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            ...
            <div class="flex flex-wrap items-center gap-2 lg:justify-end">
              <button type="button" class="btn btn-primary btn-sm" :disabled="runtimeSaving" @click="saveRuntimeConfig">
                {{ runtimeSaving ? t('admin.ops.systemLog.saving') : t('admin.ops.systemLog.saveAndApply') }}
              </button>
      ...
      <p v-if="health.last_error" class="mt-2 text-xs text-red-600 dark:text-red-400">{{ t('admin.ops.systemLog.lastWriteError') }}{{ health.last_error }}</p>
    </div>
    ...
    <div class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-5">
      ...
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ t('admin.ops.systemLog.startTimeOptional') }}
        <input v-model="filters.start_time" type="datetime-local" class="input mt-1" />
      </label>
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ t('admin.ops.systemLog.endTimeOptional') }}
    ...
    <div class="mb-3 flex flex-wrap gap-2">
      ...
      <button type="button" class="btn btn-danger btn-sm" @click="cleanupCurrentFilter">{{ t('admin.ops.systemLog.cleanupCurrent') }}</button>
      ...
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-gray-200 dark:divide-dark-700">
          <thead class="bg-gray-50 dark:bg-dark-900">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-gray-500">{{ t('admin.ops.systemLog.colTime') }}</th>
              ...
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-gray-500">{{ t('admin.ops.systemLog.colDetail') }}</th>
            </tr>
          </thead>
