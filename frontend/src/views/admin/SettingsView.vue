    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'general'" class="space-y-6">
          <GeneralSettingsTab
            v-model:backend-mode-enabled="form.backend_mode_enabled"
            v-model:site-name="form.site_name"
            v-model:site-subtitle="form.site_subtitle"
            v-model:api-base-url="form.api_base_url"
            v-model:table-default-page-size="form.table_default_page_size"
            v-model:table-page-size-options-input="tablePageSizeOptionsInput"
            v-model:custom-endpoints="form.custom_endpoints"
            v-model:contact-info="form.contact_info"
            v-model:doc-url="form.doc_url"
            v-model:site-logo="form.site_logo"
            v-model:home-content="form.home_content"
            v-model:compact-home-enabled="form.compact_home_enabled"
            v-model:hide-ccs-import-button="form.hide_ccs_import_button"
            v-model:custom-menu-items="form.custom_menu_items"
          />
        </div>
        <!-- /Tab: General -->
	        ...
	        <div v-show="activeTab === 'agreement'" class="space-y-6">
	          <LoginAgreementSettingsTab
	            v-model:enabled="form.login_agreement_enabled"
	            v-model:mode="form.login_agreement_mode"
	            v-model:updated-at="form.login_agreement_updated_at"
	            v-model:documents="form.login_agreement_documents"
	          />
	        </div>
	        <!-- /Tab: Login Agreement -->
        ...
        <EmailSettingsTab
          ...
          v-model:emailVerifyEnabled="form.email_verify_enabled"
          v-model:emailProvider="form.email_provider"
          v-model:emailApiBaseUrl="form.email_api_base_url"
          v-model:emailApiKey="form.email_api_key"
          :email-api-key-configured="form.email_api_key_configured"
          v-model:smtpHost="form.smtp_host"
          v-model:smtpPort="form.smtp_port"
          v-model:smtpUsername="form.smtp_username"
          v-model:smtpPassword="form.smtp_password"
          :smtp-password-configured="form.smtp_password_configured"
          v-model:smtpFromEmail="form.smtp_from_email"
          v-model:smtpFromName="form.smtp_from_name"
          v-model:smtpUseTls="form.smtp_use_tls"
          v-model:subscriptionExpiryNotifyEnabled="form.subscription_expiry_notify_enabled"
          v-model:balanceLowNotifyEnabled="form.balance_low_notify_enabled"
          v-model:balanceLowNotifyThreshold="form.balance_low_notify_threshold"
          v-model:balanceLowNotifyRechargeUrl="form.balance_low_notify_recharge_url"
          v-model:accountQuotaNotifyEnabled="form.account_quota_notify_enabled"
          v-model:accountQuotaNotifyEmails="form.account_quota_notify_emails"
...
import LoginAgreementSettingsTab from "@/views/admin/settings/LoginAgreementSettingsTab.vue";
...
const paymentGuideHref = computed(() =>
  locale.value.startsWith("zh")
    ...
    : "https://github.com/Redstonexs/sub2api/blob/main/docs/PAYMENT.md",
);
...
// 人机验证 UI 状态：单卡片「总开关 + 服务商单选」。Turnstile/腾讯/阿里云落库为三个
// 独立 enabled 键（与上游一致），fork 的 CAP 走 captcha_provider 选择器；下面的映射
// 保证同一时间至多一家启用。
...
function applyCaptchaSelection(provider: CaptchaProviderSelection | null): void {
  ...
  form.captcha_provider =
    provider === "cap" ? "cap" : provider === "turnstile" ? "turnstile" : "none";
}
...
// Custom menu item management moved to GeneralSettingsTab (presentation-only child).
...
.settings-tab-active::after {
  ...
  content: "";
  background: linear-gradient(90deg, #cc785c, #b05c40);
}
