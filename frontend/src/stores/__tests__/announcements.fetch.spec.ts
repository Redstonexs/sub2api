...
const notice = (id: number): UserAnnouncement => ({
  ...
  severity: 'info', show_banner: false,
...
describe('announcement fetch ownership', () => {
  it('keeps all unread announcements while rejecting a stale refresh', async () => {
    const store = useAnnouncementStore()
    const old = pendingList()
    const current = Array.from({ length: 25 }, (_, i) => notice(i + 1))
    list.mockReturnValueOnce(old.promise).mockResolvedValueOnce(current)
    const oldRequest = store.fetchAnnouncements()
    await store.fetchAnnouncements(true)
    old.resolve([notice(99)])
    await oldRequest
    expect(store.announcements).toHaveLength(25)
    expect(store.unreadCount).toBe(25)
  })

  it('preserves banner dismissals when a request from the old session completes', async () => {
    const store = useAnnouncementStore()
    const banner = { ...notice(1), show_banner: true }
    list.mockResolvedValueOnce([banner])
    await store.fetchAnnouncements()
    store.dismissBanner(banner.id)
    const old = pendingList()
    list.mockReturnValueOnce(old.promise).mockResolvedValueOnce([banner])
    const oldRequest = store.fetchAnnouncements(true)
    store.reset()
    old.resolve([])
    await oldRequest
    await store.fetchAnnouncements()
    expect(store.unreadCount).toBe(1)
    expect(store.bannerAnnouncements).toEqual([])
    expect(JSON.parse(localStorage.getItem('announcement_banner_dismissed')!))
      .toEqual({ [banner.id]: banner.updated_at })
  })
