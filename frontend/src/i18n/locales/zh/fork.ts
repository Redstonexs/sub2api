...
export default {
  ...
  // 通用 MarkdownEditor.vue 界面文案。因为组件本身是通用的，所以放在语言包根层级
  // 而非 admin.announcements 下；放在 fork.ts 是因为该文件在上游同步保留清单中。
  markdownEditor: {
    preview: "预览",
    togglePreview: "切换预览",
    previewAs: "预览效果",
    ...
    emailPreviewNote: "邮件渲染会移除原始 HTML，仅保留 Markdown 语法，且图片必须使用绝对 http(s) 地址。",
    fullscreen: "全屏",
    exitFullscreen: "退出全屏",
    shortcutHint: "Ctrl/⌘+B 加粗 · Ctrl/⌘+I 斜体 · Ctrl/⌘+K 链接",
  ...
  admin: {
    ...
    announcements: {
      ...
      form: {
        ...
        contentPlaceholder: "使用 Markdown 撰写公告，支持标题、列表、表格、链接与外链图片。",
        scheduleHint: "时间以浏览器所在时区为准。",
        severity: "严重级别",
        severityHint: "仅影响展示样式，不改变接收人群。",
        showBanner: "横幅",
        showBannerLabel: "在所有页面顶部固定展示横幅",
        showBannerHint: "与通知方式相互独立，横幅公告同样可以发送邮件。",
      },
      estimateAudience: "预估接收人数",
      audienceSummary: "{deliverable} 位接收人（命中 {matched} 位，已退订 {unsubscribed} 位）",
      audienceSummaryTruncated: "至少 {deliverable} 位接收人（命中 {matched} 位，已退订 {unsubscribed} 位）——扫描提前结束",
      failedToEstimateAudience: "预估接收人数失败。",
      sendTestEmail: "发送测试邮件给我",
      testEmailSent: "测试公告已发送至 {email}。",
      failedToSendTestEmail: "发送测试邮件失败。",
      broadcastConfirmTitle: "确认通过邮件发送该公告？",
      broadcastConfirmMessage: "将向 {deliverable} 位用户发送邮件。{suffix} 邮件发出后无法撤回。",
      broadcastConfirmTruncated: " 实际人数可能更多——受众扫描提前结束。",
      broadcastConfirmUnknown: "无法确定接收人数。发布后会向所有符合条件的用户发送邮件，且无法撤回。",
      broadcastConfirmAction: "确认发送",
      severityLabels: {
        info: "普通",
        warning: "重要",
        critical: "紧急",
      ...
      // 键名与后端错误 reason 一致，便于 extractI18nErrorMessage 直接解析
      // admin.announcements.errors.<REASON>。
      errors: {
        ANNOUNCEMENT_INPUT_REQUIRED: "缺少公告数据。",
        ANNOUNCEMENT_TITLE_INVALID: "标题不能为空，且最多 200 个字符。",
        ANNOUNCEMENT_CONTENT_REQUIRED: "公告内容不能为空。",
        ANNOUNCEMENT_CONTENT_TOO_LONG: "公告内容过长，请控制在 20000 个字符以内。",
        ANNOUNCEMENT_TIME_RANGE_INVALID: "开始时间必须早于结束时间。",
        ANNOUNCEMENT_STATUS_INVALID: "公告状态无效。",
        ANNOUNCEMENT_NOTIFY_MODE_INVALID: "通知方式无效。",
        ANNOUNCEMENT_INVALID_TARGET: "定向规则无效。",
        ANNOUNCEMENT_NOT_FOUND: "该公告已不存在。",
        TARGETING_TOO_MANY_GROUPS: "定向规则组过多，最多 50 组。",
        TARGETING_TOO_MANY_CONDITIONS: "单个规则组内条件过多，最多 50 个。",
      },
      lifecycle: {
        scheduled: "待生效",
        live: "生效中",
      ...
      unsavedChangesTitle: "放弃修改？",
      unsavedChangesMessage: "该公告有未保存的修改，关闭后将会丢失。",
      discard: "放弃",
      readCount: "已有 {count} 位用户阅读",
    },
    ops: {
      systemLog: {
        title: "系统日志",
  ...
  homeV2: {
    ...
    providerMore: '更多',
    providerSoon: '即将推出',
    leaderboardKicker: '第三方基准',
    leaderboardTitle: 'Claude 与 GPT 稳居前沿，一把密钥同时抵达。',
    leaderboardDescription: 'Artificial Analysis 用同一套九项评测为所有前沿模型打分：Claude 系列占据智能指数榜首，GPT 领跑其余阵营——顶尖的推理、Agent 编码与长上下文能力，都可通过本网关直接调用。',
    leaderboardViewLive: '查看实时榜单',
    leaderboardScoreLabel: 'Artificial Analysis 智能指数，满分 100',
    leaderboardScoreUnit: '/ 100',
    leaderboardRoutable: '本网关可路由',
    leaderboardSnapshot: '数据快照日期 {date}',
    leaderboardNote: '同一模型家族仅保留得分最高的配置；原榜单将每档推理强度单列一行。',
    leaderboardStatLeaderLabel: '指数榜首',
    leaderboardStatOpenAiLabel: 'OpenAI 最高分模型',
    leaderboardStatEvalsLabel: '聚合评测数量',
