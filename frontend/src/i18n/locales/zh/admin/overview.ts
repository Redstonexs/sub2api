export default {
    ...
    dashboard: {
      ...
      groupQuotaCard: {
        title: '分组额度总览',
        selectGroup: '选择分组',
        noViewableGroups: '暂无可查看的分组',
        totalRemaining5h: '总5h剩余',
        totalRemaining7d: '总7d剩余',
        perAccountUsage: '账号用量',
        accountLabel: '账号{n}',
        noData: '—',
        noAccounts: '该分组下暂无账号',
        sortBy5h: '按5h剩余排序',
        ...
        loadFailed: '加载配额数据失败',
        resetsAt: '{time} 重置',
        grantManagement: '查看权限管理',
        grantedUsers: '已授权用户',
        addUserById: '按用户ID添加',
        add: '添加',
        revoke: '移除',
        noGrantedUsers: '暂无授权用户',
        grantSuccess: '查看权限已授予',
        revokeSuccess: '查看权限已移除',
        grantFailed: '授予访问权限失败',
        revokeFailed: '移除访问权限失败',
        grantedBy: '授权人：{name}',
        grantedAt: '授权时间：{time}',
      }
    },
    ...
    groups: {
      ...
      form: {
        ...
        qosEnabled: 'QoS 降级阶梯',
        qosEnabledHint: '用户在本分组的消耗越过档位阈值后，逐级改发更便宜的模型，而不是到达硬限额时直接拒绝。启用后才开始计数，不追溯历史消耗。',
        qosMetric: '阈值计量口径',
        qosMetricHint: '原价口径按未打折价格累计，折扣再深也无法掩盖真实消耗；扣费口径按用户实际支付金额累计。',
        qosMetricOption: {
          list: '未打折原价',
          charged: '实际扣费（消耗余额）',
        },
        qosTiers: '档位',
        qosTiersHint: '按严重程度升序排列，命中的最高档位整档生效。',
        qosTiersEmpty: '暂无档位。添加一个以开始对重度用户降级。',
        qosTierLabel: '第 {index} 档',
        addQosTier: '添加档位',
        removeQosTier: '删除档位',
        qosWindow: '统计窗口',
        qosWindowOption: {
        ...
        qosThreshold: '阈值（USD）',
        qosNoChange: '不改变',
        qosMaxReasoningEffort: '限制推理强度',
        qosRpmLimit: '限制每分钟请求数',
        qosBlock: '阻断请求',
        qosBlockHint: '最高档：直接拒绝请求，直到窗口重置。',
        qosModelMappings: '模型改写',
        qosModelFrom: '请求模型',
        qosModelTo: '实发模型',
        qosModelFromPlaceholder: 'gpt-5.6-sol*（结尾 * 表示前缀匹配）',
        qosModelToPlaceholder: 'gpt-5.6-luna',
        addQosModelMapping: '添加改写',
        removeQosModelMapping: '删除改写',
        qosError: {
          windowRequired: '请选择统计窗口',
          thresholdInvalid: '请输入不小于 0 的阈值',
          thresholdNotAscending: '同一窗口下，阈值必须大于上一档',
          noAction: '该档位至少需要配置一项动作',
          tooManyTiers: '档位过多（最多 16 个）',
          tooManyMappings: '改写过多（最多 64 条）',
          rpmInvalid: '请输入不小于 0 的整数',
          effortUnsupported: '当前平台不支持推理强度',
          fromRequired: '请输入请求模型',
          toRequired: '请输入实发模型',
          duplicateFrom: '同一档位内请求模型不能重复',
          wildcardTarget: '实发模型不能包含通配符',
        },
        exclusiveLabel: '专属分组',
