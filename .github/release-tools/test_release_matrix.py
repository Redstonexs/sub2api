...
class ReleaseMatrixTest(unittest.TestCase):
    def setUp(self):
        ...
        release.VERSION_FILE.write_text('1.8.1\n')
    ...
    def test_plan_accepts_fork_tags_and_checks_the_selected_commit(self):
        for tag in ('1.8.1', 'v1.8.1'):
            with self.subTest(tag=tag), patch.dict(os.environ, {'GITHUB_OUTPUT': 'outputs'}), patch.object(
                ...
                self.assertEqual(output['version'], '1.8.1')
                self.assertEqual(release.VERSION_FILE.read_text(), '1.8.1\n')

    def test_plan_rejects_upstream_tags_and_non_fork_versions(self):
        for tag in ('v0.2.7', '0.2.7', '9.8.7', '1.8.1-rc1', 'main'):
            with self.subTest(tag=tag), patch.object(subprocess, 'check_output', return_value='a' * 40 + '\n'):
                with self.assertRaisesRegex(ValueError, 'fork 1.x.y'):
                    release.plan(argparse.Namespace(ref=tag, dry_run=False, simple=False))
                self.assertEqual(release.VERSION_FILE.read_text(), '1.8.1\n')

    def test_dry_run_rejects_an_upstream_version_file(self):
        release.VERSION_FILE.write_text('0.2.7\n')
        with patch.object(subprocess, 'check_output', return_value='a' * 40 + '\n'):
            with self.assertRaisesRegex(ValueError, 'invalid fork VERSION'):
    ...
    def test_dry_run_plan_resolves_matrix_without_a_new_tag(self):
        ...
        self.assertEqual(output['tag'], '1.8.1')
        self.assertEqual(output['owner_lower'], 'exampleowner')
        self.assertEqual(len(json.loads(output['matrix'])['include']), 5)

    def test_publication_retries_only_transient_failures(self):
        fake_bin = Path('bin')
        fake_bin.mkdir()
        command = fake_bin / 'publish'
...
''')
        ...
        sleep.chmod(0o755)
        cases = (
            ('secondary rate limit', 1, 0, 2, ['180']),
            ('HTTP status code 503', 3, 17, 3, ['180', '360']),
            ('unauthorized: authentication required', 3, 17, 1, []),
            ('invalid config', 3, 17, 1, []),
        )
        for i, (message, failures, code, attempts, waits) in enumerate(cases):
            with self.subTest(message=message):
                attempts_path = Path(f'attempts-{i}').resolve()
                sleep_path = Path(f'sleep-{i}').resolve()
                env = {**os.environ, 'PATH': str(fake_bin.resolve()) + os.pathsep + os.environ['PATH'],
                       'RUNNER_TEMP': self.temp.name, 'ATTEMPTS': str(attempts_path),
                       'SLEEP_LOG': str(sleep_path), 'FAIL_COUNT': str(failures), 'FAIL_MESSAGE': message}
                result = subprocess.run(['bash', str(ROOT / '.github/release-tools/retry-transient.sh'), 'publish'],
                                        env=env, capture_output=True, text=True)
                self.assertEqual(result.returncode, code, result.stdout + result.stderr)
                self.assertEqual(int(attempts_path.read_text()), attempts)
                self.assertEqual(sleep_path.read_text().splitlines() if sleep_path.exists() else [], waits)
