...
FORK_VERSION_RE = re.compile(r'1\.\d+\.\d+')
...
def plan(args):
    ...
    if args.dry_run:
        ...
        if not FORK_VERSION_RE.fullmatch(version):
            raise ValueError('publishing requires a fork 1.x.y release version tag (optional v prefix)')
    ...
    if not FORK_VERSION_RE.fullmatch(version):
        raise ValueError('invalid fork VERSION: expected 1.x.y')
