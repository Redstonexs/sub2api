...
The frontend reuses the default branch CI artifact cache when its sources match. Each binary job restores the fork's cross-compilation cache warmed by `release-cache.yml` on the default branch; tag builds remain restore-only so they do not evict reusable caches. Matrix jobs upload uniquely named artifacts. The publishing job extracts only the regular Linux binary from each verified archive and restores its executable permission before constructing Docker contexts. QEMU remains limited to runtime-image instructions. DockerHub images are omitted when its credentials are absent; GHCR is always retained. Simple mode still publishes only the amd64 GHCR image and the simple release description.

Fork publication accepts only `1.x.y` tags (with an optional `v` prefix for manual runs), verifies that the checked-out commit matches the tag, and rejects upstream version lines. Docker publication and release uploads keep the fork's bounded retry policy for transient registry/API failures; authentication and configuration failures stop immediately.
...
bash -n .github/release-tools/retry-transient.sh
```
