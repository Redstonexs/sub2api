...
func TestSettingHandler_GetPublicSettings_ExposesCAPConfigurationWithoutSecret(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h := NewSettingHandler(service.NewSettingService(&settingHandlerPublicRepoStub{
		values: map[string]string{
			service.SettingKeyCaptchaProvider: "cap",
			service.SettingKeyCapAPIEndpoint:  "https://cap.example.com",
			service.SettingKeyCapSiteKey:      "public-site-key",
			service.SettingKeyCapSecretKey:    "server-secret",
		},
	}, &config.Config{}), "test-version")
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.Equal(t, "cap", resp.Data.CaptchaProvider)
	require.Equal(t, "https://cap.example.com", resp.Data.CapAPIEndpoint)
	require.Equal(t, "public-site-key", resp.Data.CapSiteKey)
	require.Empty(t, resp.Data.CapSecretKey)
}
