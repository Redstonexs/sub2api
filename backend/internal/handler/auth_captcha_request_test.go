...
func TestAuthRequestsBindTencentCaptchaProof(t *testing.T) {
	...
	proof := captchaProof(login.TurnstileToken, login.TencentCaptchaTicket, login.TencentCaptchaRandstr, login.CaptchaToken)
	...
	proof = captchaProof(pending.TurnstileToken, pending.TencentCaptchaTicket, pending.TencentCaptchaRandstr, pending.CaptchaToken)
	require.Equal(t, "ticket-value", proof.TencentTicket)
	require.Equal(t, "@rand-value", proof.TencentRandstr)
}
