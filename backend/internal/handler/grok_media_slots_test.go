...
// The fork gates video status on a separate requestID-only owner key, so a lookup
// resolves two distinct session keys: the account binding and the owner binding.
func (s *grokMediaSlotBindings) GetSessionAccountID(_ context.Context, groupID int64, key string) (int64, error) {
	if groupID != 24 {
		return 0, service.ErrStickySessionNotFound
	}
	if owner, ok := s.keys[key]; ok {
		return owner, nil
	}
	return 0, service.ErrStickySessionNotFound
}
...
func newGrokMediaSlotHandler(t *testing.T, oauth, mismatch bool, platforms ...string) (*OpenAIGatewayHandler, *grokMediaSlotsCache, *grokMediaSlotBindings, *grokMediaSlotUpstream) {
	...
	// The fork 404s a video status lookup before it spends a slot unless the task owner is bound.
	require.NoError(t, gateway.BindGrokMediaVideoRequestOwner(context.Background(), &groupID, "task", 10))
	bindings.writes = 0
