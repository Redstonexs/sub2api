...
func TestGeminiChatCompatReasoningPricingDoesNotInferLevelFromBudget(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, stream := range []bool{false, true} {
		mode := "buffered"
		if stream {
		...
		t.Run(mode, func(t *testing.T) {
			...
			require.Equal(t, http.StatusOK, recorder.Code)
			require.Len(t, upstream.requestBodies, 1)
			// The fork forwards the converted thinking budget. Upstream's new
			// pricing policy applies only to explicit Gemini thinking levels.
			require.True(t, gjson.GetBytes(upstream.requestBodies[0], "generationConfig.thinkingConfig.includeThoughts").Bool())
			require.Positive(t, gjson.GetBytes(upstream.requestBodies[0], "generationConfig.thinkingConfig.thinkingBudget").Int())
			require.False(t, gjson.GetBytes(upstream.requestBodies[0], "generationConfig.thinkingConfig.thinkingLevel").Exists())
			require.Nil(t, result.ReasoningEffort)
