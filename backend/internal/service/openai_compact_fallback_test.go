...
func TestPrepareOpenAICompactFallbackRetryPreservesNativeTriggerAndContext(t *testing.T) {
	...
	pathBefore, pathBeforeErr := openAIResponsesRequestPathSuffix(c)
	require.NoError(t, pathBeforeErr)
	...
	pathAfter, pathAfterErr := openAIResponsesRequestPathSuffix(c)
	require.NoError(t, pathAfterErr)
...
func TestPrepareOpenAICompactFallbackRetryLegacyPathAndSingleAttemptGuard(t *testing.T) {
	...
	require.True(t, retry)
	require.Equal(t, "gpt-5.4", fallbackModel)
	compactSuffix, compactSuffixErr := openAIResponsesRequestPathSuffix(c)
	require.NoError(t, compactSuffixErr)
	require.Equal(t, "/compact", compactSuffix)
