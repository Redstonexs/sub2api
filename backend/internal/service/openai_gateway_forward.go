...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if account.UsesOpenAICodexProtocol() {
		...
		// 上游 v0.1.18x 引入：在任何 body 改写之前留存客户端原始 prompt_cache_key，
		// 供下方 header 侧只派生一次命名空间（本 fork 的会话对齐同样会改写该字段）。
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	if account.UsesOpenAICodexProtocol() {
		...
		apiKeyID := getAPIKeyIDFromContext(c)
		// 账号 namespace（上游 v0.1.18x）：先按选中账号隔离客户端自带的身份头，
		// 避免 scheduler failover 把同一组 Codex IDs 发给另一份 OAuth 凭据。
		// 必须排在会话对齐之前——session/thread/window 这几个载体随后会被
		// applyCodexSessionIdentityHeaders 统一改写成同一个 UUID（本 fork 的档案对齐），
		// 这里只负责 installation_id / x-client-request-id / turn_id 等它不接管的字段。
