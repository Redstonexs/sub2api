...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	// 账号 namespace（上游 v0.1.18x）：与 buildUpstreamRequest 同序，排在会话对齐之前。
	// session/thread/window 载体随后由 applyCodexSessionIdentityHeaders 统一改写成同一个
	// UUID（本 fork 的档案对齐）；放在之后会用已派生值当种子再套一层，两侧载体就不再自洽。
