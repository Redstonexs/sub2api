...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	if account.Platform == PlatformOpenAI {
		policyBody, changed, policyErr := ApplyOpenAIReasoningEffortPolicyFromContextWithGroupQoSForModel(ctx, responsesBody, normalizedModel)
		if policyErr != nil {
	...
	// 身份收口由 buildUpstreamRequest 内的 enforceCodexIdentityHeadersWithUA 直接完成：
	// bridge 分支不再删除 originator，因此那个卫语句不会再提前返回，也就不需要事后补回
	// （事后补回只有本调用方做，其余调用方遇到 bridge 形态 body 就会漏出不完整身份）。
	// 判定谓词跟随上游放宽为 UsesOpenAICodexProtocol()，以覆盖走 Codex 协议的非 OAuth 账号。
