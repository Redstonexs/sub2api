...
// ProvideAntigravityOAuthService wires the settings-backed credential
// resolver. The constructor remains compatible with direct tests and the
// generated graph until Wire is regenerated by the parent lane.
func ProvideAntigravityOAuthService(
	...
	if settingService != nil {
		settingService.SetAntigravityOAuthEncryptor(encryptor)
	}
	svc := NewAntigravityOAuthService(proxyRepo)
	if settingService != nil {
		svc.SetCredentialsResolver(settingService.ResolveAntigravityOAuthClientCredentials)
	}
	return svc
}
