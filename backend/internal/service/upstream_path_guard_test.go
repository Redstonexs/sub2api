...
// responsesSuffixOrEmpty 适配本 fork 的 (string, error) 签名：不合规的子路径不得
// 贡献任何上游后缀，因此出错时折算成空串再断言。
func responsesSuffixOrEmpty(c *gin.Context) string {
...
func TestOpenAIResponsesRequestPathSuffixRejectsNonConformingSubpaths(t *testing.T) {
	...
	for _, path := range nonConformingPaths {
		t.Run(path, func(t *testing.T) {
			...
			require.Empty(t, responsesSuffixOrEmpty(c),
				"path %q must never contribute an upstream path suffix", path)
			require.Equal(t, chatgptCodexURL,
				appendOpenAIResponsesRequestPathSuffix(chatgptCodexURL, responsesSuffixOrEmpty(c)))
