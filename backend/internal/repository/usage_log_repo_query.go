...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		groupQoSTier              sql.NullInt64
		groupQoSWindow            sql.NullString
		groupQoSEffectMask        sql.NullInt64
	...
	// Group QoS 快照：三列总是同时写入，tier 为 NULL 即表示无生效档位
	// （历史行 / fail-open）。window 与 mask 缺失时保守回退为空值与 0。
	if groupQoSTier.Valid {
		...
		log.GroupQoSRecord = &service.GroupQoSRecordSnapshot{
