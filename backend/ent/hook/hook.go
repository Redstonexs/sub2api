...
// The GroupViewGrantFunc type is an adapter to allow the use of ordinary
// function as GroupViewGrant mutator.
type GroupViewGrantFunc func(context.Context, *ent.GroupViewGrantMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f GroupViewGrantFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GroupViewGrantMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GroupViewGrantMutation", m)
}
