...
func (_q *UserQuery) QueryGroupViewGrants() *GroupViewGrantQuery {
...
func (_q *UserQuery) WithGroupViewGrants(opts ...func(*GroupViewGrantQuery)) *UserQuery {
	query := (&GroupViewGrantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withGroupViewGrants; query != nil {
		if err := _q.loadGroupViewGrants(ctx, query, nodes,
			func(n *User) { n.Edges.GroupViewGrants = []*GroupViewGrant{} },
			func(n *User, e *GroupViewGrant) { n.Edges.GroupViewGrants = append(n.Edges.GroupViewGrants, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadGroupViewGrants(ctx context.Context, query *GroupViewGrantQuery, nodes []*User, init func(*User), assign func(*User, *GroupViewGrant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(groupviewgrant.FieldUserID)
	}
	query.Where(predicate.GroupViewGrant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.GroupViewGrantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
