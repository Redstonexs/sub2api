...
type GroupQuery struct {
	...
	withGroupViewGrants   *GroupViewGrantQuery
...
// QueryGroupViewGrants chains the current query on the "group_view_grants" edge.
func (_q *GroupQuery) QueryGroupViewGrants() *GroupViewGrantQuery {
	query := (&GroupViewGrantClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withGroupViewGrants:   _q.withGroupViewGrants.Clone(),
...
// WithGroupViewGrants tells the query-builder to eager-load the nodes that are connected to
// the "group_view_grants" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithGroupViewGrants(opts ...func(*GroupViewGrantQuery)) *GroupQuery {
	query := (&GroupViewGrantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGroupViewGrants = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withGroupViewGrants; query != nil {
		if err := _q.loadGroupViewGrants(ctx, query, nodes,
			func(n *Group) { n.Edges.GroupViewGrants = []*GroupViewGrant{} },
			func(n *Group, e *GroupViewGrant) { n.Edges.GroupViewGrants = append(n.Edges.GroupViewGrants, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadGroupViewGrants(ctx context.Context, query *GroupViewGrantQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupViewGrant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(groupviewgrant.FieldGroupID)
	}
	query.Where(predicate.GroupViewGrant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.GroupViewGrantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
